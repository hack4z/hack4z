...
export default function Card({card}){
	...
	const [bookmark, setBookmark] = useState(card.isBookmarked);
			...
			{ window.innerWidth >= '800' ? (
				<div>
					<div key={card.id} className="relative flex cursor-pointer w-[760px] h-32 2xl:w-[960px] 2xl:h-44 mx-auto my-6 border border-solid border-white/20 rounded-md">
						<img src={card.img} alt="" className="shrink-0 h-full rounded-md" />
						<div className="grid grid-col-1">
							<div className="flex">
								<h1 className="text-2xl text-violet-700 hover:underline bold ml-2 mt-2">{card.name}</h1>
								<p className="cursor-default py-2 px-4 mt-1 text-center ">{card.created_at} {card.time}</p>
							</div>
